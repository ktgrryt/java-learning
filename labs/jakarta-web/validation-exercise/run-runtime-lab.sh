...
fail_all() {
  for id in validation-accepts validation-rejects validation-cross-field validation-error-body; do
...
if [ ! -s target/orders-api.war ] \
    || ! jar tf target/orders-api.war | grep -Fq 'WEB-INF/classes/cafe/api/OrderResource.class'; then
  show "$runtime_dir/build.log"
  fail_all 'OrderResourceを含むorders-api.warがありません'
fi
...
cp target/orders-api.war target/liberty/wlp/usr/servers/defaultServer/apps/orders-api.war
...
printf '%s\n' '--- 見るところ（答えは出しません） ---'
printf '%s\n' '制約を宣言しただけになっていないか（Resource側で有効にする1語が要る）'
printf '%s\n' '1項目では判定できないルールを、まとめて見る形で宣言しているか'
printf '%s\n' '400の本文に、どの項目がなぜ悪いかが入っているか'
printf '%s\n' 'getPropertyPath() をそのまま返していないか（arg0 は内部の作り）'
