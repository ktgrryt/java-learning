...
fail_all() {
  for id in rest-public rest-unauthenticated rest-no-leak rest-forbidden; do
...
if [ ! -s target/secure-api.war ] \
    || ! jar tf target/secure-api.war | grep -Fq 'WEB-INF/classes/cafe/api/OrderResource.class'; then
  show "$runtime_dir/build.log"
  fail_all 'OrderResourceを含むsecure-api.warがありません'
fi
...
cp target/secure-api.war target/liberty/wlp/usr/servers/defaultServer/apps/secure-api.war
...
printf '%s\n' '--- 見るところ（答えは出しません） ---'
printf '%s\n' '守るべき場所ごとに「誰が呼べるか」を宣言しているか（無い場所は公開）'
printf '%s\n' '401（誰か分からない）と403（権限が無い）の違い'
printf '%s\n' '内部の形をそのまま返していないか（社内メモが一緒に出ていく）'
